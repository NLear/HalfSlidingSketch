...
Sliding_Bloom::Sliding_Bloom(uint _STAGE_BYTE, uint _HASH_NUM, uint _lo = 3) :
...
//    uint offset = Clock_Pos % STAGE_BIT;
    for (int i = 0; i < LO; ++i) {
        uint query_level = (l_max + i) % HASH_NUM;
        uint position = hash[query_level].run(key, KEY_LEN) % STAGE_BIT + query_level * STAGE_BIT;
        if (!future->Get(position))
            return false;
    }
    return true;
}
