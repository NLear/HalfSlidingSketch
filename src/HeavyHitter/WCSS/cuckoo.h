...
map<string, int> cuckoo::get_map() {
    map<string, int> kv_map;
    for (int i = 0; i < hash_num; i++) {
        for (int j = 0; j < seg_size; j++) {
            kv_map[array[i][j].ID] = array[i][j].val;
        }
    }
    return kv_map;
}
...
pair<kv, int> cuckoo::kick_iter(kv n_kv,
                                int former_address) // try to insert a kv, if not success, randomly select one position to kick, the kick position can not be the same as the former position
