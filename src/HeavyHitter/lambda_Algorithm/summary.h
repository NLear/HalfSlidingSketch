...
public:
    ...
    map<Data, int> GetHeavy(long long int num);
};
...
Summary::Summary(int num, int dl, int _cycle, int _para_lambda) {
    ...
    datalen = dl;
    counter_size = 4 + 8 * 5 +
                   dl;    // 1 counter, 2 pointer for double linked list, 1 pointer to bucket, 2 pointer for queue, head and tail, and 1 ID
    cycle = _cycle;
...
Sketch::Sketch(int num, int dl, int _cycle, int _para_lambda, int _cycle_i) {
    ...
    int hash_kv_size =
            datalen + 8 * 2; //1 key, 1 value which is a counter pointer, and 1 pointer for linked list of hash table;
    MAX_SIZE -= (num / (counter_size + hash_kv_size)) * 1.25 *
                hash_kv_size; // the memory of hash table, allocated according to the max number of counter; In the worst case, each counter has value smaller than lamda, the queue is empty, and most counters have the same value, the bucket number is
...
map<Data, int> Summary::GetHeavy(long long int num) {
    map<Data, int> ret;
    map<Data, bool> all_flows;
    for (int i = 0; i < 2; i++) {
        for (auto &it : S[i]->counter_map) {
            Data data = it.first;
            if (all_flows.find(data) == all_flows.end()) {
                all_flows[data] = true;
                int est_val = Summary::Query(data, num);
                if (est_val > PHI) {
                    ret.insert(make_pair(data, est_val));
                }
            }
        }
    }
