...
void Error_Rate(int memory) {
    const int hash_number = 6, field_num = 1, query_hash_number = 3;
    const int row_length = (memory * 1024) / hash_number / (4 * field_num);

    Recent_Counter CM_Counter(CYCLE, row_length, hash_number, field_num, query_hash_number);
    Recent_Counter CU_Counter(CYCLE, row_length, hash_number, field_num, query_hash_number);
    Recent_Counter CO_Counter(CYCLE, row_length, hash_number, field_num, query_hash_number);
...
//    cout << memory << " " << ave_err_CM / tests << " " << ave_err_CU / tests << " " << ave_err_CO / tests << endl;
    cout << memory << " " << ave_err_CM / tests << endl;
    cout << memory << " " << ave_err_CU / tests << endl;
    cout << memory << " " << ave_err_CO / tests << endl;
    fclose(file);
    delete[] dat;
}
...
void speed(int memory) {
    const int hash_number = 8, field_num = 1, query_hash_number = 3;
    const int row_length = (memory * 1024) / hash_number / (4 * field_num);
